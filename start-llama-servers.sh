...
# Run llama server hosting LLM with reasoning disabled
nohup "$LLAMA_SERVER_EXEC" -m "$LLAMACPP_PATH_MODEL" -c "$LLAMACPP_MODEL_CONTEXT_SIZE" -ngl "$LLAMACPP_MODEL_NGL" -fa on --port "$LLAMA_CPP_MODEL_PORT" -np "$LLAMACPP_MODEL_NUM_PARALLEL_SLOTS" --reasoning "$LLAMACPP_MODEL_REASONING" --chat-template-kwargs "{\"enable_thinking\": ${LLAMACPP_MODEL_THINKING:-false}}" > $HOME/thesis-project/sample-project/logs/model.log 2>&1 &
# Run llama server hosting LLM with reasoning enabled (open models_config.py to configure reasoning level)
...
nohup "$LLAMA_SERVER_EXEC" -m "$LLAMACPP_PATH_EMBEDDINGS_MODEL" -c "$LLAMACPP_EMBED_CONTEXT_SIZE" -ngl "$LLAMACPP_EMBED_NGL" --port "$LLAMA_CPP_EMBED_PORT" --embedding --pooling "$LLAMACPP_EMBED_POOLING"  > $HOME/thesis-project/sample-project/logs/embedding.log 2>&1 &
