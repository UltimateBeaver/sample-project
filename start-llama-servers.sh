...
nohup "$LLAMA_SERVER_EXEC" -m "$LLAMACPP_PATH_EMBEDDINGS_MODEL" -c "$LLAMACPP_EMBED_CONTEXT_SIZE" -ngl "$LLAMACPP_EMBED_NGL" --port "$LLAMA_CPP_EMBED_PORT" --embedding > $HOME/thesis-project/sample-project/logs/embedding.log 2>&1 &
